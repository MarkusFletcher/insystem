const getTemplate = (options = [], placeholder) => {
	placeholder = placeholder ?? options[0].text
...
export class Select {
	...
	render() {
		...
		this.$element.insertAdjacentHTML('beforeEnd', getTemplate(this.options, this.placeholder))
	}

	setup() {
		if(this.placeholder) this.$select.value = ''
	...
	moveClasses() {
		const classList =  [...this.$select.classList]
